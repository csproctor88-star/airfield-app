...
export function AcsiDiscrepancyPicker({ onSelect, onClose }: AcsiDiscrepancyPickerProps) {
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 12px' }}>
          {!loading && filtered.length === 0 && (
            <div style={{ padding: 24, textAlign: 'center', color: 'var(--color-text-3)', fontSize: 'var(--fs-sm)' }}>
              {search ? 'No discrepancies match your search.' : 'No discrepancies found for this installation.'}
            </div>
          )}
          ...
          {filtered.map(d => (
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                border: '1px solid var(--color-border)',
                background: linking === d.id ? 'rgba(56, 189, 248, 0.08)' : 'var(--color-bg-elevated)',
            ...
            >
              ...
              {d.work_order_number && (
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--color-text-4)', marginTop: 2 }}>
