...
// Try to load the manifest — will be {} if not yet generated
let wildlifeImageManifest: Record<string, { filename: string; source_url: string; license: string; group: string }> = {}
try {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  wildlifeImageManifest = require('../public/wildlife_image_manifest.json')
} catch {
  // Manifest not yet generated — will use remote URLs
}
...
export function resolveWildlifeImage(species: WildlifeSpecies): string | null {
  const entry = wildlifeImageManifest[species.common_name]
  if (entry?.filename) {
