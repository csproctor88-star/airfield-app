...
// Loaded at runtime only — the file may not exist until the scraper is run.
// Uses a lazy-load pattern to avoid build-time errors when the JSON is absent.
...
function getWildlifeImageManifest(): Record<string, ManifestEntry> {
  ...
  if (typeof window !== 'undefined') {
    // Client-side: manifest isn't available via require, use remote URLs
    _manifestCache = {}
    return _manifestCache
  }
  try {
    // Server-side: try reading the manifest file
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const fs = require('fs')
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const path = require('path')
    const manifestPath = path.join(process.cwd(), 'public', 'wildlife_image_manifest.json')
    if (fs.existsSync(manifestPath)) {
      _manifestCache = JSON.parse(fs.readFileSync(manifestPath, 'utf-8'))
    } else {
      _manifestCache = {}
    }
  } catch {
