...
// PDF. Single-apron exports pass a one-element array with no label; multi-
// apron exports pass one entry per Add Apron capture.
...
export async function generateParkingPdf(input: ParkingPdfInput): Promise<{ doc: jsPDF; filename: string }> {
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    if (section.label) {
      ...
      y += 7
      // Per-section subtotal line — small, matches description style.
      doc.setFontSize(8)
      doc.setTextColor(80)
  ...
  // date (typical multi-apron workflow) don't overwrite each other on download.
