...
export async function parseAmtrRecordWorkbook(buf: ArrayBuffer): Promise<ParsedRecord> {
  ...
  const out: ParsedRecord = { cover: {}, qtp: [], quals: [], jqs: [], r1098: {}, items797: [], e623a: [], rat: [], items803: {} }
  ...
  // DAF 1098 (current + prior year sheets)
  for (const [sheet, year] of [['DAF Form 1098 2026', '2026'], ['DAF Form 1098 2025', '2025']] as [string, string][]) {
...
export type ImportSummary = { counts: Record<string, number>; unmatched: string[] }
 ...
 * and any catalog rows that couldn't be matched. */
export async function applyAmtrImport(
  ...
  for (const item of p.qtp) {
    ...
    await upsertAmtrRow('amtr_qual_progress', { base_id: installationId, member_id: memberId, catalog_id: c.id, complete_date: item.complete_date || null, attained: !!item.complete_date })
    written++
  }
  for (const item of p.quals) {
    const c = qualByName.get(norm(item.name))
    if (!c) { unmatched.push(`Qual: ${item.name}`); continue }
    await upsertAmtrRow('amtr_qual_progress', { base_id: installationId, member_id: memberId, catalog_id: c.id, attained: item.attained })
    written++
  }
  ...
  for (const [year, rows] of Object.entries(p.r1098)) for (const row of rows) {
    ...
    await upsertAmtrRow('amtr_1098_progress', { base_id: installationId, member_id: memberId, catalog_id: c.id, year_label: year, start_date: row.start_date || null, last_completed: row.last_completed || null, next_due: next, certifier_initials: row.certifier || null, trainee_initials: row.trainee || null })
    written++
  }
  ...
  for (const row of p.jqs) {
    ...
    await upsertAmtrRow('amtr_jqs_progress', { base_id: installationId, member_id: memberId, catalog_id: c.id, start_date: row.start_date || null, complete_date: row.complete_date || null, trainee_initials: row.trainee || null, trainer_initials: row.trainer || null, certifier_initials: row.certifier || null })
    written++
  }
  ...
  for (const row of p.rat) {
    ...
    await upsertAmtrRow('amtr_rat_progress', { base_id: installationId, member_id: memberId, catalog_id: c.id, completed: row.completed || null })
    written++
  }

  // Free-form inserts (797, 623A, 803).
  if (p.items797.length) {
    await insertAmtrRows('amtr_797', p.items797.map((it, i) => ({ base_id: installationId, member_id: memberId, task: it.task, start_date: it.start_date || null, complete_date: it.complete_date || null, trainee_initials: it.trainee || null, trainer_initials: it.trainer || null, certifier_initials: it.certifier || null, milestone_window: it.milestone_window || null, requires_certifier: !!it.certifier, sort_order: i })))
    written += p.items797.length
  }
  if (p.e623a.length) {
    await insertAmtrRows('amtr_623a', p.e623a.map((e) => {
    ...
    written += p.e623a.length
  }
  for (const [section, rows] of Object.entries(p.items803)) {
    await insertAmtrRows('amtr_803', rows.map((it, i) => ({ base_id: installationId, member_id: memberId, section, sts_item: it.sts_item || null, eval_date: it.eval_date || null, in_ugt: it.in_ugt || null, results: it.results || null, evaluator_initials: it.evaluator || null, sort_order: i })))
    written += rows.length
  }
