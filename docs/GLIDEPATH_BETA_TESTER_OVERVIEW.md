...
It works like an app on your phone but runs in your web browser. You can add it to your home screen and it even works offline.
...
**What you can do:**
- Report a new discrepancy with photos, GPS location on a map, and a description
...
2. Tap each item: **Pass** (green), **Fail** (red), or **N/A** (gray)
3. Use **Mark All Pass** to quickly clear routine sections
4. BWC items have a special four-level toggle: LOW / MOD / SEV / PROHIB
5. Your progress auto-saves — if your browser closes, your draft is still there
6. When finished, tap **Complete** to mark your half done
7. A filer combines the Airfield and Lighting halves and **Files** the official daily report
8. Export the combined report as a PDF with pass/fail summaries and embedded photos
...
- **Daily Operations Summary** — Everything that happened on a given day or date range: inspections, checks, status changes, discrepancies, evaluations. Includes photos and maps.
...
Every report exports as a **branded PDF** with your installation name, embedded photos, satellite maps, page numbers, and timestamps.
...
**How it works:**
1. Tap a location on the satellite map (or enter coordinates)
2. Enter the object height
3. The app automatically evaluates against **all 10 imaginary surfaces** for **every runway** at your base
4. See instantly which surfaces (if any) are violated, by how many feet, with the exact UFC table reference

**The 10 surfaces evaluated:**
Primary, Approach-Departure, Transitional, Inner Horizontal, Conical, Outer Horizontal, Clear Zone, Graded Area, APZ I, APZ II
...
Look up hundreds of military and civilian aircraft with detailed specifications.
...
- Use the **ACN/PCN Comparison Panel** to check whether a specific aircraft is safe for your runway
...
- Export the full waiver register as an Excel workbook with multiple sheets
...
- **Delete accounts** (sys_admin only, with confirmation)
...
- **Profile** — View your name, email, rank, role, and assigned base
- **Base Configuration** (admin) — Set up runways, NAVAIDs, airfield areas, CE shops, and upload your airfield diagram
- **Inspection Templates** (admin) — Customize your base's inspection checklists: add, edit, remove, and reorder sections and items
...
2. **Inspection workflow** — Is the checklist easy to work through on a phone or tablet in the field? Does the tailorable template system cover your base's needs?
3. **Discrepancy tracking** — Does the lifecycle (Open → CES → Work Completed → Closed) match how your shop actually works?
4. **Check types** — Are the fields for each check type capturing the right information? Is anything missing?
5. **Reports** — Are the four report types useful for your briefings? What additional data would help?
6. **Obstruction evaluations** — Do the results make sense for obstructions you've evaluated before? Are the UFC references accurate?
7. **References** — Is the 70-entry library covering the regulations you need? Is offline caching useful?
8. **Waivers** — Does the AF-505 field set capture everything you track? Is the coordination workflow realistic?
9. **Performance** — How does the app feel on your device? Any slowness, crashes, or confusing behavior?
10. **Missing features** — What do you wish the app did that it doesn't?
...
| Discrepancies | Bottom nav → More → Discrepancies, or Dashboard quick action |
| Airfield Checks | Bottom nav → More → Airfield Checks, or Dashboard quick action |
| Daily Inspections | Bottom nav → More → Inspections, or Dashboard quick action |
| Reports | Bottom nav → More → Reports |
| Obstructions | Bottom nav → Obstruction Eval |
| Aircraft Database | Bottom nav → Aircraft |
| References | Bottom nav → Regulations |
| Waivers | Bottom nav → More → Waivers |
| NOTAMs | Bottom nav → More → NOTAMs |
| Activity Log | Bottom nav → More → Activity Log |
| Settings | Bottom nav → More → Settings |
| User Management | Bottom nav → More → User Management (admin only) |
