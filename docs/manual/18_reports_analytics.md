...
- **PDF** — the matrix as one flat table (all rows together, not split into the on-screen sections): each person is one row, unlinked rows carry an explicit **(unlinked)** suffix in the User column, followed by a Totals row and the same coverage footnotes as the screen. Selecting more than seven domains shrinks the table to fit and truncates long column names and unlinked names — the full, untruncated names are always in the Excel export.
- **Excel** — a **Summary** sheet with the full matrix (no truncation) and a filterable **Category** column (Personnel / Unlinked / Unattributed) that stands in for the on-screen section grouping, plus one additional sheet per selected domain listing that domain's underlying records (person, record, date) for a deeper audit than the on-screen drill-down allows.
