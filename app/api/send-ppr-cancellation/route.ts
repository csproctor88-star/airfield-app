...
export async function POST(request: Request) {
  try {
    ...
    // ppr:write (the permission the cancel action requires) plus base access.
    const authorized = await callerCanActOnPpr(reader, user.id, entry.base_id, PPR_EMAIL_PERMS.cancellation)
    if (!authorized) {
