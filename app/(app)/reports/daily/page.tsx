...
import { ArrowLeft, FileText, Download, Calendar, Mail, Loader2 } from 'lucide-react'
...
export default function DailyOpsPage() {
  ...
  const handleExport = async () => {
    if (!data) return
    setExporting(true)
    const { doc, filename } = generateDailyOpsPdf(data, makePdfOpts())
    doc.save(filename)
    setExporting(false)
  }
  ...
  const handleEmailPdf = async () => {
    if (!data) return
    setExporting(true)
    const result = generateDailyOpsPdf(data, makePdfOpts())
