...
import { fetchWaivers, fetchReviewsByYear, createWaiverReview, type WaiverRow, type WaiverReviewRow } from '@/lib/supabase/waivers'
...
export default function AnnualReviewYearPage() {
  ...
  const handleSaveReview = async (waiverId: string) => {
    ...
    if (error) {
      ...
      // Reload reviews
      if (installationId) {
        const r = await fetchReviewsByYear(installationId, year)
        setReviews(r)
      }
    }
    setSaving(null)
  ...
  return (
    <div style={{ padding: 16, paddingBottom: 100 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <Link href="/waivers/annual-review" style={{ color: 'var(--color-cyan)', fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>
          &larr; Annual Review
        </Link>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, justifyContent: 'center' }}>
        <button onClick={() => router.push(`/waivers/annual-review/${year - 1}`)}
        ...
        <button onClick={() => router.push(`/waivers/annual-review/${year + 1}`)} disabled={year >= currentYear}
      ...
      <div style={{ fontSize: 11, color: 'var(--color-text-3)', textAlign: 'center', marginBottom: 16 }}>
        {reviewedIds.size} of {activeWaivers.length} waivers reviewed
      </div>
            ...
            <div key={w.id} className="card" style={{ marginBottom: 8, borderLeft: `3px solid ${reviewed ? '#22C55E' : '#EF4444'}` }}>
              ...
              {isExpanded && (
                <div style={{ marginTop: 12, borderTop: '1px solid var(--color-border)', paddingTop: 12 }}>
                  {reviewed && (
                    <div style={{ marginBottom: 8, padding: '6px 8px', background: '#22C55E11', borderRadius: 6, fontSize: 11, color: '#22C55E' }}>
                      Already reviewed for {year}. Submitting again will create a duplicate entry.
                    </div>
                  )}
      ...
      {!loading && activeWaivers.length === 0 && (
        <div className="card" style={{ textAlign: 'center', padding: 24, color: 'var(--color-text-3)', fontSize: 13 }}>No active waivers for {year} review</div>
      )}
    </div>
  )
}
