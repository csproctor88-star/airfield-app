/**
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║  AOMS — Regulation PDF Library with react-pdf                   ║
 * ║  Supabase Storage → IndexedDB Cache → react-pdf Viewer          ║
 * ╚══════════════════════════════════════════════════════════════════╝
 *
 *  SETUP (run these in your Next.js project):
 *  ──────────────────────────────────────────
 *  npm install react-pdf
 *
 *  ⚠️  CRITICAL: react-pdf requires the PDF.js worker. There are
 *  two ways to configure it. Both are included below — pick one.
 *
 *  OPTION A — CDN worker (simplest, works immediately):
 *    Already set below with pdfjs.GlobalWorkerOptions.workerSrc
 *
 *  OPTION B — Local worker (better for offline / airgapped):
 *    1. npm install pdfjs-dist
 *    2. Copy node_modules/pdfjs-dist/build/pdf.worker.min.mjs
 *       to your public/ folder
 *    3. Change workerSrc to "/pdf.worker.min.mjs"
 *
 *  NEXT.JS SPECIFIC — Add to next.config.js if you get build errors:
 *  ─────────────────
 *  module.exports = {
 *    webpack: (config) => {
...
// ─── PDF.js Worker Setup ─────────────────────────────────────
// This is the #1 reason react-pdf silently fails.
// Match the version EXACTLY to your react-pdf's bundled pdfjs version.
// react-pdf v9.x uses pdfjs 4.x; react-pdf v7.x uses pdfjs 3.x
// Check: node -e "console.log(require('react-pdf/package.json').dependencies['pdfjs-dist'])"
pdfjs.GlobalWorkerOptions.workerSrc = `//unpkg.com/pdfjs-dist@${pdfjs.version}/build/pdf.worker.min.mjs`;

// ─── CRITICAL: Import the CSS for text/annotation layers ─────
// Without these, PDFs render but text selection and links break.
// If using Next.js App Router, import in your layout.tsx instead:
...
// ─────────────────────────────────────────────────────────────
// Minimal Supabase Storage Client (or use @supabase/supabase-js)
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// IndexedDB Helpers (zero-dependency localforage replacement)
// ─────────────────────────────────────────────────────────────
...
/**
 * ⚠️ KEY FIX: Convert Blob → ArrayBuffer for react-pdf.
 * Passing a blob: URL or raw Blob to react-pdf's `file` prop
 * will silently fail or throw. It needs { data: Uint8Array }.
 */
...
export default function PDFLibrary() {
  ...
  const [pdfData, setPdfData] = useState(null); // Uint8Array for react-pdf
    ...
    try {
      if (navigator.onLine) {
        ...
        if (cached) {
          ...
          const keys = await idbGetAllKeys(STORE_BLOBS);
          setFiles(keys.map((k) => ({ name: k, id: k, metadata: {} })));
        }
      }
    } catch (e) {
      setError(e.message);
  ...
  const viewPdf = useCallback(
    async (fileName) => {
      ...
      try {
        // Try cache first
        let blob = await idbGet(STORE_BLOBS, fileName);
        ...
        // ⚠️ KEY: Convert to Uint8Array — this is what react-pdf needs
        const uint8 = await blobToUint8Array(blob);
        setPdfData(uint8);
      } catch (e) {
        setPdfError(`Failed to load: ${e.message}`);
      } finally {
        setPdfLoading(false);
      }
    },
  ...
  function onDocumentLoadError(err) {
    ...
    setPdfError(
      `PDF render failed: ${err.message}. Check the worker config — this is almost always a worker version mismatch.`
    );
  }
  ...
  return (
    <div style={S.root}>
        ...
        <div style={S.headerRow}>
          <div style={S.headerLeft}>
            ...
            <div>
              ...
              <p style={S.subtitle}>
                {files.length} regulation{files.length !== 1 ? "s" : ""} ·{" "}
          ...
          <div
            style={{
              ...S.badge,
              ...(isOnline ? S.badgeOnline : S.badgeOffline),
            }}
          >
            <span
              style={{
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button
            ...
            style={{
              ...S.btn,
              ...S.btnAccent,
              ...(!isOnline || cachedCount === files.length ? S.btnOff : {}),
            }}
          >
            ↓ Cache All
          </button>
      ...
      {error && (
        <div style={S.error}>
          <span>{error}</span>
          <button onClick={() => setError(null)} style={S.errorX}>
            ✕
          </button>
        </div>
      )}
        ...
        {viewingFile ? (
          ...
          <div style={S.viewer}>
            ...
            <div style={S.viewerBar}>
              <button onClick={closeViewer} style={S.backBtn}>
                ← Back
              </button>
              ...
              <div style={S.viewerControls}>
                ...
                <button
                  ...
                  style={{
                    ...S.ctrlBtn,
                    ...(currentPage >= (numPages || 1) ? S.btnOff : {}),
                  }}
                >
                  ›
                </button>
              ...
              {pdfError && (
                <div style={S.center}>
                  <div style={S.pdfErrBox}>
                    ...
                    >
                      Troubleshooting: Check the browser console. The most
                      common cause is a PDF.js worker version mismatch. Run:{" "}
                      <code style={S.code}>
                        node -e
                        "console.log(require('pdfjs-dist/package.json').version)"
                      </code>{" "}
                      and make sure workerSrc matches.
                    </p>
                  </div>
                </div>
              )}
          ...
          <div style={{ ...S.center, flexDirection: "column", padding: 80 }}>
            ...
            <p style={{ color: "#475569", fontSize: 13, marginTop: 4 }}>
              {searchQuery
                ...
                : `Verify bucket "${BUCKET_NAME}" contains PDF files`}
            </p>
          </div>
        ) : (
          /* ═══════ FILE LIST ═══════ */
      ...
      <footer style={S.footer}>
        <span>
          Supabase Storage · <code style={S.code}>{BUCKET_NAME}</code>
        </span>
        <span>
          react-pdf v{pdfjs.version} · IndexedDB offline cache
        </span>
      </footer>
    </div>
  );
...
// ─────────────────────────────────────────────────────────────
// Styles — Dark slate / utilitarian command-center
// ─────────────────────────────────────────────────────────────
const S = {
  root: {
    fontFamily:
